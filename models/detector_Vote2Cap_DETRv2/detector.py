...
class Model_Vote2Cap_DETR(nn.Module):
    ...
    def run_encoder(self, point_clouds, point_cloud_dims):
        ...
        enc_xyz, enc_features, enc_inds = self.encoder(
            pre_enc_features,
            scale_ranges=torch.stack(point_cloud_dims, dim=1),
            xyz=pre_enc_xyz
        )
        if enc_inds is None:
    ...
    def forward(self, inputs, is_eval: bool=False):
        ...
        enc_xyz, enc_features, enc_inds = self.run_encoder(point_clouds, point_cloud_dims)
